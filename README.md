...
# or: cargo run -p oah -- dev --fresh
```

Then open [http://127.0.0.1:43147](http://127.0.0.1:43147). The console talks to the `support-desk` agent. Try:

> Customer cannot export invoices, ticket 42

The scripted model calls `lookup_ticket`, then drafts a reply. History is a Flue v1 snapshot. Live updates are SSE.

One-shot CLI (exit `0` completed, `1` failed):

```bash
cargo run -p oah -- run support-desk -m "Ticket 42: CSV export is broken" --id ticket-42
```

```bash
cargo run -p oah -- migrate
cargo run -p oah -- inspect support-desk/ticket-42
cargo run -p oah -- dev --bind 127.0.0.1:43147 --fresh
```

`oah dev` starts the local host with `MockModel`. `--fresh` deletes the SQLite file first.
